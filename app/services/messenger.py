...
class Messenger:
    ...
    def birthday_message(self, student_name, age=None):
        ...
        return (
            f"Happy Birthday, {student_name}! 🎂{age_text}\n"
            f"On your special day, we at Guha India wish you joy, good health and success in all you do.\n"
            f"May this new year of your life bring you closer to your dreams and fill every step with achievement.\n"
            f"We are proud to have you as part of our family. Here's to a year of growth and bright opportunities!\n"
            f"Warm wishes,\n"
            f"Guha India 🌟"
        )
    ...
    def send_birthday_wish(self, student_name, phone, age=None):
        cfg = self._get_settings()
        if not cfg['whatsapp_token'] or not cfg['whatsapp_phone_id']:
            return False
        # Most approved 'birthday_wish' templates have a single {{1}} placeholder
        # for the name, so only send that to the template.
        ok, _ = self._send_whatsapp(phone, 'birthday_wish', [student_name])
        if ok:
            return True
        # Fallback for cases where the template isn't approved yet (only valid
        # inside Meta's 24h free-form window; business-initiated messages must
        # use a template).
        return self._send_sms_direct(phone, self.birthday_message(student_name, age))

    def batch_birthday_wishes(self):
        ...
        recipients = []
        for s in students:
            if not s.date_of_birth or not s.phone:
                continue
            if (s.date_of_birth.month, s.date_of_birth.day) == (today.month, today.day):
                age = today.year - s.date_of_birth.year
                recipients.append((s.name, s.phone, {'age': age}))
        if not recipients:
            return {'sent': 0, 'failed': 0, 'errors': ['No birthdays today']}
        results = {'sent': 0, 'failed': 0, 'errors': []}
        for name, phone, extra in recipients:
            ok = self.send_birthday_wish(name, phone, extra.get('age'))
            if ok:
                results['sent'] += 1
            else:
                results['failed'] += 1
