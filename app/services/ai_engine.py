...
class AIEngine:
    ...
    def _call_gemini(self, prompt, timeout=60, first_only=False):
        ...
        for model in models:
            try:
                ...
                print(f"Gemini model {model} failed (HTTP {response.status_code})")
            except Exception as e:
                print(f"Gemini API Error ({model}): {e}")
        return None

    def _call_openai(self, prompt, timeout=60):
    ...
    def generate_predictive_analytics(self, stats):
        # Deterministic rule-based baseline that guarantees a well-formed response.
        ...
        if stats.get('low_attendance_count', 0) > current_students * 0.2:
            base["risk_factors"].append("Significant number of at-risk students with poor attendance")
        if stats.get('enquiries_new', 0) > stats.get('enquiries_converted', 0):
            base["opportunities"].append("High enquiry volume presents conversion opportunity")
        if current_revenue > 0:
            base["opportunities"].append("Strong cash flow enables course expansion")
        if stats.get('tutors', 0) < stats.get('courses', 0):
            base["opportunities"].append("Opportunity to hire specialized tutors for new courses")
        base["risk_factors"] = base["risk_factors"] or ["No significant risks identified"]
        base["opportunities"] = base["opportunities"] or ["Focus on marketing to drive growth"]

        # Cache identical results so repeated dashboard loads are instant and the
        # slow/rate-limited AI key never blocks the panel.
        global _pa_cache
        ...
        try:
            ai_response = self._call_gemini(prompt, timeout=6, first_only=True) \
                or self._call_openai(prompt, timeout=6)
        except Exception as e:
            print(f"Predictive analytics AI error: {e}")
            ai_response = None
        if ai_response:
            try:
                cleaned = ai_response.strip()
                ...
                if isinstance(parsed, dict):
                    merged = dict(base)
                    for key in ('enrollment_forecast', 'revenue_forecast'):
                        val = parsed.get(key)
                        if isinstance(val, list) and len(val) >= 3 and all(isinstance(v, (int, float)) for v in val[:3]):
                            merged[key] = [int(v) for v in val[:3]]
                    growth = parsed.get('growth_rate')
                    if isinstance(growth, (int, float)):
                        merged['growth_rate'] = round(float(growth), 1)
                    for key in ('risk_factors', 'opportunities', 'course_recommendations'):
                        val = parsed.get(key)
        ...
        _pa_cache["signature"] = signature
        _pa_cache["data"] = result
        _pa_cache["time"] = now
        return result
    ...
    def generate_todays_tasks(self, data):
        ...
        # Serve a cached result for identical data to keep this endpoint fast and
        ...
        try:
            ...
            print(f"Today's tasks AI call error: {e}")
        tasks = base
        if ai_response:
            try:
                cleaned = ai_response.strip()
        ...
        _tasks_cache["signature"] = signature
